 ...
 * 菜单状态变化时会自动触发相应的动画： FloatButton.js:95-98
        // 主要动画类型
        // 1. 平移动画 (Translation) Anim.js:24-26
        // 系统使用 translationX、translationY、translationZ 实现按钮的位置移动动画，这是悬浮按钮显示、隐藏和菜单展开的核心动画效果。        
        // 2. 透明度动画 (Alpha) Anim.js:27
        // 通过 alpha 属性控制按钮的透明度变化，实现淡入淡出效果。        
        // 3. 缩放动画 (Scale) Anim.js:28-29
        // 缩放动画，但主要用于按钮状态切换时的视觉反馈。        
        // 4. 按钮状态切换动画 Anim.js:206-210
        // 在按钮状态变化时，系统会同时执行缩放动画（从 1 → 0.7 → 1）和颜色变化动画，创造出按钮被"按压"的视觉效果。        
        // 5. 停靠动画 (Direction) Anim.js:135-147
        // 使用 ValueAnimator 和 BounceInterpolator 实现悬浮按钮自动停靠到屏幕边缘的弹性动画效果。        
        // 6. 菜单展开动画 Anim.js:237-241
        // 菜单按钮的展开使用组合动画，包括位置移动和缩放效果的同步执行。        
        // 动画组合机制 Anim.js:30-32
        // 系统通过 AnimatorSet 将多个动画组合在一起同时播放，创造出复杂的视觉效果。        
        // Notes
        // Anim.js 实现的是一套完整的悬浮按钮动画系统，包含平移、缩放、透明度、颜色变化等多种动画效果的组合。虽然包含缩放动画，但平移动画是最主要的效果，用于实现按钮的显示/隐藏和菜单展开/收起。 
 * @LastEditors: 大柒
